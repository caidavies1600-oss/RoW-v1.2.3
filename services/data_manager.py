...
class DataManager:
    ...
    def _live_sync_file(self, filepath: str, data: Any):
        """Create templates for manual data entry (no auto-sync of actual data)."""
        try:
            filename = os.path.basename(filepath)

            if filename == "events.json":
                self.sheets_manager.sync_current_teams(data)
                logger.info("🔄 Updated current teams template")
            # Note: Other files will have templates created but not auto-synced

        except Exception as e:
            logger.warning(f"Failed to update template for {filepath}: {e}")
    ...
    def save_player_stats(self):
        ...
        success = self.save_json("data/player_stats.json", self.player_stats)
        if success:
