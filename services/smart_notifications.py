...
class QuietHoursModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("❌ Please enter valid numbers for hours", ephemeral=True)
...
class NotificationsCog(commands.Cog):
    ...
    async def show_settings(self, ctx):
        ...
        if hasattr(ctx, 'interaction') and ctx.interaction:
            ...
            await ctx.interaction.response.send_message(embed=view.children[0], view=view, ephemeral=True)
        else:
            # This is a regular command
            await ctx.send(embed=discord.Embed(title="Loading...", color=COLORS["INFO"]), view=view)
            await view.update_embed(type('MockInteraction', (), {'response': type('MockResponse', (), {'is_done': lambda: False})(), 'edit_original_response': ctx.edit})())
