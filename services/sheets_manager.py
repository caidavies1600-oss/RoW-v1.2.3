            ...
            class SheetsManager:
                ...
                # Include ALL the template methods from your paste.txt version
                ...
                # Add the other template methods from your paste.txt here...
            ...
            class SheetsManager:
                """Core Google Sheets integration for Discord bot."""
                ...
                async def scan_and_sync_all_members(self, bot, guild_id: int = None):
                    ...
                    try:
                        ...
                        # Sync to sheets
                        sync_success = self._sync_to_sheets(new_members, updated_members)
                ...
                def _sync_to_sheets(self, new_members, updated_members):
                    """Sync member data to Google Sheets."""
                    try:
                        worksheet = self._get_or_create_player_sheet()
                ...
                async def full_sync_and_create_templates(self, bot, all_data: dict, guild_id: int = None):
                    """Complete sync: members + basic templates."""
                    if not self.spreadsheet:
                        return {"success": False, "error": "Sheets not initialized"}
                    ...
                    try:
                        ...
                        templates_created = self._create_basic_templates(all_data)
                ...
                def _create_basic_templates(self, all_data):
                    """Create essential templates only."""
                    try:
                        success_count = 0

                        # Current teams
                        if self._create_current_teams_sheet(all_data.get("events", {})):
                            success_count += 1

                        # Results history
                        if self._create_results_sheet(all_data.get("results", {})):
                            success_count += 1

                        # Basic dashboard
                        if self._create_simple_dashboard():
                            success_count += 1
                ...
                def _create_current_teams_sheet(self, events_data):
                    """Create current teams sheet."""
                    try:
                        try:
                            worksheet = self.spreadsheet.worksheet("Current Teams")
                            worksheet.clear()
                        ...
                        for team_key, players in events_data.items():
                            team_name = team_mapping.get(team_key, team_key)
                            player_list = ", ".join(str(p) for p in players) if players else ""
                            worksheet.append_row([timestamp, team_name, len(players), player_list, "Active"])

                        return True
                    except:
                        return False

                def _create_results_sheet(self, results_data):
                    """Create results history sheet."""
                    try:
                        try:
                            worksheet = self.spreadsheet.worksheet("Results History")
                            worksheet.clear()
                ...
                def _create_simple_dashboard(self):
                    ...
                    try:
                        ...
                        worksheet.update('A3', 'Team Performance Summary')
                        worksheet.update('A5', 'Recent Activity')
                        worksheet.update('A10', 'Manual charts can be added here')

                        return True
                    except:
                        return False

            # Legacy compatibility - keep existing method names that other code might use
                def sync_current_teams(self, events_data):
                    return self._create_current_teams_sheet(events_data)

                def create_all_templates(self, all_data):
                    return self._create_basic_templates(all_data)