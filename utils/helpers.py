...
from config.constants import TEAM_DISPLAY, EMOJIS
from config.settings import ALERT_CHANNEL_ID
import logging


def days_until_expiry(expiry_timestamp: str, duration_days: int) -> int:
    """Calculate how many days remain until expiry based on timestamp and duration."""
    try:
...
class Helpers:
    ...
    @staticmethod
    def format_time_remaining(blocked_at_str: str, duration_days: int = 7) -> str:
        """Format time remaining until expiry based on blocked_at timestamp and duration."""
        try:
    ...
    @staticmethod
    def days_until_expiry(expiry_timestamp: str, duration_days: int) -> int:
        """Calculate days until expiry - wrapper for the standalone function."""
