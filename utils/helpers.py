...
@staticmethod
def format_time_remaining(blocked_user_info: dict) -> str:
    """Format time remaining until expiry from blocked user info."""
    try:
        blocked_at = blocked_user_info.get("blocked_at")
        duration = blocked_user_info.get("ban_duration_days", 0)

        if not blocked_at:
            return "Invalid"

        blocked_time = datetime.fromisoformat(blocked_at)
        expiry = blocked_time + timedelta(days=duration)
        ...
        logging.warning(f"Error formatting time remaining: {e}")
        return "Invalid"

@staticmethod
def days_until_expiry(blocked_at_str: str, duration_days: int) -> int:
    """Calculate days until a block expires."""
    try:
        ...
        logging.warning(f"Error calculating days until expiry: {e}")
        return 0
        ...
        for channel in guild.text_channels:
            ...
            if channel.id == ALERT_CHANNEL_ID:
                try:
                    ...
                    logging.error(f"Failed to create fake context in channel {channel.name}: {e}")
                    continue

    raise ValueError("⚠️ ALERT_CHANNEL_ID does not match any accessible channel in this guild.")
```