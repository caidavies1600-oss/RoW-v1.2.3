...
class AdminNotifier:
    ...
    async def update_startup_progress(self, milestone: str, status: str = "✅", details: str = None):
    ...
    async def send_startup_complete(self, success: bool, total_time: float, stats: Dict[str, Any]):
        ...
        try:
            ...
            title = "✅ Bot Started Successfully!" if success else "❌ Bot Startup Failed"
    ...
    async def send_error_alert(self, error_type: str, error: Exception, context: str = None, traceback_str: str = None):
    ...
    async def send_activity_notification(self, activity_type: str, details: Dict[str, Any]):
        ...
        try:
            ...
            activity_config = {
                "command_executed": {"color": 0x5DADE2, "emoji": "⚡", "title": "Command Executed"},
                "user_blocked": {"color": 0xED4245, "emoji": "🚫", "title": "User Blocked"},
                "user_unblocked": {"color": 0x57F287, "emoji": "✅", "title": "User Unblocked"},
                "event_started": {"color": 0x5865F2, "emoji": "📢", "title": "Event Started"},
                "result_recorded": {"color": 0xFEE75C, "emoji": "🏆", "title": "Result Recorded"},
                "auto_task": {"color": 0x6C757D, "emoji": "🤖", "title": "Automated Task"},
                "data_sync": {"color": 0x17A2B8, "emoji": "🔄", "title": "Data Sync"},
                "critical_error": {"color": 0xED4245, "emoji": "🚨", "title": "Critical Error"}
            }

            config = activity_config.get(activity_type, {"color": 0x6C757D, "emoji": "ℹ️", "title": "Bot Activity"})
            ...
            for key, value in details.items():
                if key == "user" and isinstance(value, discord.User):
                    ...
                    embed.add_field(
                        name="⏱️ Duration",
                        value=f"{value} days" if activity_type == "user_blocked" else f"{value:.2f}s",
                        inline=True
                    )
    ...
    async def send_health_alert(self, health_status: Dict[str, Any]):
        ...
        try:
            ...
            if "cog_status" in health_status:
                cog_issues = [name for name, status in health_status["cog_status"].items() 
...
async def notify_startup_complete(success: bool, total_time: float, stats: Dict[str, Any]):
