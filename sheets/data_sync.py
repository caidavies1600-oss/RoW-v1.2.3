...
class EnhancedDataSync(TemplateCreator):
        ...
        try:
            ...
            loading_tasks = [
                ...
                (
                    ...
                    self._load_notification_preferences_enhanced,
                ),
            ]
            ...
            self.log_usage_stats()
            logger.info(
                f"✅ Enhanced data load complete: {successful_loads}/{len(loading_tasks)} successful"
            )
                ...
                try:
                    ...
                    if team_key and team_key in data["events"]:
                        if players_str and players_str.strip():
                            ...
                            logger.debug(
                                f"Loaded {len(players)} players for {team_key}"
                            )
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    user_id = str(
                        row.get("User ID", "") or row.get("👤 User ID", "")
                    ).strip()
                    ...
                    ign = (
                        row.get("IGN", "")
                        or row.get("🎮 In-Game Name", "")
                        or row.get("Name", "")
                    )
                    display_name = (
                        row.get("Display Name", "")
                        or row.get("📱 Discord Name", "")
                        or f"User_{user_id}"
                    )

                    # Parse role and status
                    main_team_role = (
                        str(row.get("Main Team Role", "No") or "No").lower() == "yes"
                    )
                    blocked = (
                        str(
                            row.get("Blocked Status", "No")
                            or row.get("Blocked", "No")
                            or "No"
                        ).lower()
                        == "yes"
                    )
                    ...
                    specializations = {
                        "cavalry": str(row.get("Cavalry", "No") or "No").lower()
                        == "yes",
                        "mages": str(row.get("Mages", "No") or "No").lower() == "yes",
                        "archers": str(row.get("Archers", "No") or "No").lower()
                        == "yes",
                        "infantry": str(row.get("Infantry", "No") or "No").lower()
                        == "yes",
                        "whale": str(row.get("Whale Status", "No") or "No").lower()
                        == "yes",
                    }
                    ...
                    logger.warning(
                        f"Failed to parse player row for user {user_id}: {e}"
                    )
                    continue

            logger.info(
                f"📊 Loaded {len(data['player_stats'])} player profiles from sheets"
            )
            return True
    ...
    def _load_results_history_enhanced(self, data: dict) -> bool:
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    normalized_team = team_mapping.get(
                        team.lower().strip(), team.lower().replace(" ", "_")
                    )
            ...
            logger.info(
                f"📊 Loaded {len(history)} results ({total_wins}W/{total_losses}L)"
            )
            return True
        ...
        try:
            ...
            def _get_notification_data():
                try:
                    ...
                    logger.info(
                        "Notification Preferences sheet not found, using defaults"
                    )
                    return []
            ...
            for row in rows:
                try:
                    ...
                    event_reminders = parse_bool(
                        row.get("Event Reminders", "") or row.get("📢 Event Alerts", "")
                    )
                    result_notifications = parse_bool(
                        row.get("Result Notifications", "")
                        or row.get("🏆 Result Notifications", "")
                    )
                    team_updates = parse_bool(
                        row.get("Team Updates", "") or row.get("👥 Team Updates", "")
                    )
                    dm_notifications = parse_bool(
                        row.get("DM Notifications", "")
                        or row.get("📱 DM Notifications", "")
                    )

                    # Parse timing preferences
                    reminder_time_str = (
                        row.get("Reminder Times", "")
                        or row.get("🕐 Reminder Time", "")
                        or "60 minutes"
                    )
                    try:
                    ...
                    # Parse timezone
                    timezone_str = (
                        row.get("Timezone", "") or row.get("🌍 Timezone", "") or "UTC+0"
                    )
                    try:
                    ...
                    logger.warning(
                        f"Failed to parse notification preferences for {user_id}: {e}"
                    )
                    continue
            ...
            logger.info(
                f"📊 Loaded notification preferences for {len(users_prefs)} users"
            )
            return True
        ...
        try:
            ...
            def _setup_notification_sheet():
                try:
                    ...
                    worksheet = self.spreadsheet.add_worksheet(
                        title="Notification Preferences", rows="200", cols="10"
                    )
            ...
            for user_id, prefs in users_data.items():
                try:
                    ...
                    event_alerts = (
                        "✅ Enabled"
                        if merged_prefs.get("event_reminders", True)
                        else "❌ Disabled"
                    )
                    result_notifications = (
                        "✅ Enabled"
                        if merged_prefs.get("result_notifications", True)
                        else "❌ Disabled"
                    )
                    team_updates = (
                        "✅ Enabled"
                        if merged_prefs.get("team_updates", True)
                        else "❌ Disabled"
                    )
                    dm_notifications = (
                        "✅ Enabled"
                        if merged_prefs.get("method", "channel") in ["dm", "both"]
                        else "❌ Disabled"
                    )

                    reminder_times = merged_prefs.get("reminder_times", [60, 15])
                    reminder_time = (
                        f"{reminder_times[0]} minutes"
                        if reminder_times
                        else "60 minutes"
                    )
                    ...
                    timezone = (
                        f"UTC{timezone_offset:+d}" if timezone_offset != 0 else "UTC+0"
                    )

                    quiet_hours = merged_prefs.get(
                        "quiet_hours", {"start": 22, "end": 8}
                    )
                    quiet_hours_str = (
                        f"{quiet_hours['start']:02d}:00-{quiet_hours['end']:02d}:00"
                    )
                    ...
                    logger.warning(
                        f"Failed to prepare notification data for {user_id}: {e}"
                    )
                    continue
            ...
            logger.info(
                f"✅ Enhanced notification preferences sync: {len(users_data)} users"
            )
            return True
    ...
    def validate_loaded_data(self, data: dict) -> bool:
        ...
        try:
            ...
            if (
                not isinstance(results["total_wins"], int)
                or not isinstance(results["total_losses"], int)
                or not isinstance(results["history"], list)
            ):
...
class DataSync(TemplateCreator):
    ...
    def sync_all_data(self, all_data):
        ...
        try:
            ...
            logger.info(
                f"✅ Data sync completed: {success_count}/3 operations successful"
            )
