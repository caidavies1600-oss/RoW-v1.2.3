        ...
        class RowBot(commands.Bot):
            ...
            async def setup_hook(self):
                ...
                try:
                    ...
                    # Run data integrity checks before loading cogs
                    from utils.data_manager import DataManager
            ...
            async def _load_cogs(self):
                ...
                cog_order = [
                    # Core systems first
                    "cogs.user.profile",           # IGN management (needed by events)
                    "cogs.events.manager",         # Event management core
                    "cogs.events.results",         # Results tracking
                    "cogs.events.alerts",          # Event alerts/reminders

                    # Admin tools
                    "cogs.admin.actions",          # Block/unblock users
                    "cogs.admin.attendance",       # Absence tracking
                    "cogs.admin.exporter",         # Data export tools
                    "cogs.admin.owner_actions",    # Owner-only commands

                    # User interfaces
                    "cogs.interactions.buttons",   # Button interactions
                    "cogs.interactions.dropdowns", # Dropdown menus
                    "cogs.user.commands",          # Public user commands
                ]
                ...
                # Don't fail startup if some non-critical cogs fail
            ...
            async def on_command_error(self, ctx, error):
                """Called when a command encounters an error (fallback)."""
                # This is handled by error_handler.py, but we log here as backup
            ...
            async def close(self):
                ...
                try:
                    ...
                    pass  # Tasks might not be running
