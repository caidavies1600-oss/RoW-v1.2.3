...
class RowBot(commands.Bot):
    ...
    async def _load_cogs(self):
            ...
            for cog in cogs:
                ...
                try:
                    ...
                    if cog in critical_cogs:
                        logger.critical(f"🚨 Critical cog {cog} failed to load! Bot may not function properly.")
                        print(f"DEBUG: CRITICAL COG FAILURE: {cog}")
            
            logger.info(f"✅ {tier_name}: {tier_loaded} loaded, {tier_failed} failed")
            print(f"DEBUG: {tier_name} complete - {tier_loaded} loaded, {tier_failed} failed")
        ...
        success_rate = (len(loaded_cogs) / total_attempted * 100) if total_attempted > 0 else 0
        
        logger.info(f"📊 Cog loading summary: {len(loaded_cogs)}/{total_attempted} loaded ({success_rate:.1f}% success)")
        print(f"DEBUG: Final summary - {len(loaded_cogs)}/{total_attempted} cogs loaded ({success_rate:.1f}% success)")
        ...
        if failed_cogs:
            ...
            if failure_analysis:
                ...
                for tier, failed_list in failure_analysis.items():
                    logger.warning(f"   {tier}: {len(failed_list)} failed - {', '.join(failed_list)}")
        ...
        missing_critical = [name for name, cog in critical_verification.items() if not cog]
        if missing_critical:
