...
class Results(commands.Cog):
    ...
    def save_results(self):
        ...
        success = self.data_manager.save_json(FILES["RESULTS"], self.results, sync_to_sheets=True)
        if not success:
    ...
    def update_player_stats_for_result(self, team_key: str, result: str, players: list):
        ...
        try:
            ...
            for player_id in players:
                ...
                self.data_manager.update_player_stats(player_id, team_key, result, player_name)
            
            # Save updated player stats
            self.data_manager.save_player_stats()
            logger.info(f"Updated player stats for {len(players)} players: {team_key} {result}")
    ...
    async def record_win(self, ctx, team_key: str):
        ...
        if self.save_results():
            ...
            if notifications_cog:
                await notifications_cog.smart_notifications.notify_match_result(team_key, True, current_players)
            ...
            logger.info(f"{ctx.author} recorded win for {team_key} with {player_count} players")
        else:
    ...
    async def record_loss(self, ctx, team_key: str):
        ...
        if self.save_results():
            ...
            notifications_cog = self.bot.get_cog("NotificationsCog")
            if notifications_cog:
                await notifications_cog.smart_notifications.notify_match_result(team_key, False, current_players)
            team_display = TEAM_DISPLAY.get(team_key, team_key)
            player_count = len(current_players)
            ...
            await ctx.send(embed=embed)
            logger.info(f"{ctx.author} recorded loss for {team_key} with {player_count} players")
        else:
    ...
    async def show_results(self, ctx):
        ...
        for entry in self.results.get("history", [])[-10:]:
            try:
                ...
                recent_results.append(f"{emoji} {team} ({player_count} players) — {date}")
            except Exception as e:
    ...
    async def create_sheets_templates(self, ctx):
        ...
        try:
            await ctx.send("🔄 Creating Google Sheets templates with current players...")
            ...
            if self.data_manager.create_all_templates(all_data):
                sheets_url = self.data_manager.sheets_manager.spreadsheet.url if self.data_manager.sheets_manager.spreadsheet else "Check console for URL"
                embed = discord.Embed(
                ...
                await ctx.send("⚠️ Some templates may not have been created. Check logs for details.")
