...
from config.constants import EMOJIS, TEAM_DISPLAY, MAIN_TEAM_ROLE_ID
...
class JoinButton(Button):
    def __init__(self, team_key: str, label: str, emoji: str):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Check role permission for main team - ensure we have a Member object
        if self.team_key == "main_team":
            if not isinstance(interaction.user, discord.Member):
                await interaction.response.send_message(
                    f"{EMOJIS['ERROR']} Unable to verify permissions.",
                    ephemeral=True
                )
                return
        ...
        if hasattr(bot, 'get_cog'):
            ...
            if profile_cog and hasattr(profile_cog, 'has_ign') and profile_cog.has_ign(interaction.user):
...
class LeaveButton(Button):
    def __init__(self):
        super().__init__(style=discord.ButtonStyle.danger, label="Leave Team", emoji="❌")
