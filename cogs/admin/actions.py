...
class AdminActions(commands.Cog):
    ...
    async def check_json_files(self, ctx):
        """Check status of all JSON data files."""
        try:
            import json
            expected_files = {
                "events": FILES["EVENTS"],
                "ign_map": FILES["IGN_MAP"],
                "events_history": FILES["EVENTS_HISTORY"],
                "row_times": FILES["ROW_TIMES"],
                "absent_users": FILES["ABSENT_USERS"]
            }
                ...
                if os.path.exists(file_path):
                    try:
                        with open(file_path, 'r') as f:
                            data = json.load(f)
                        if data:
                            file_status.append(f"✅ {file_key}: Valid JSON ({len(data)} items)")
                        else:
                            file_status.append(f"⚠️ {file_key}: Empty/Invalid JSON")
                    except Exception as e:
                        file_status.append(f"❌ {file_key}: JSON Parse Error - {str(e)}")
                else:
                    file_status.append(f"❌ {file_key}: File Missing")
            ...
            embed.set_footer(text=f"Checked {len(expected_files)} files")
            await ctx.send(embed=embed)

        except Exception as e:
            await ctx.send(f"❌ Error in checkjson: {str(e)}")
