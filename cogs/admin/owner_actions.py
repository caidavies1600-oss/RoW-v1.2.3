...
class OwnerActions(commands.Cog):
            ...
            if not isinstance(data, dict):
                ...
                for team in ("main_team", "team_2", "team_3"):
                    if team not in data:
                        ...
                        for idx, user_id in enumerate(data[team]):
                            if not isinstance(user_id, int) or user_id <= 0:
                                issues.append(f"Invalid user ID in {team}[{idx}]: {user_id}")
    ...
    async def fix_json_files(self, ctx: commands.Context):
        ...
        for key, path in FILES.items():
            ...
            if needs_fix:
                ...
                if isinstance(default, dict) and isinstance(data, dict):
                    if key == "EVENTS":
                        default["main_team"] = [uid for uid in data.get("main_team", []) if isinstance(uid, int) and uid > 0]
                        default["team_2"] = [uid for uid in data.get("team_2", []) if isinstance(uid, int) and uid > 0]
                        default["team_3"] = [uid for uid in data.get("team_3", []) if isinstance(uid, int) and uid > 0]
                    elif key == "BLOCKED":
        ...
        if fixed:
            embed.add_field(name="⚠️ Note", value="Defaults applied; you may need to reconfigure settings.", inline=False)
