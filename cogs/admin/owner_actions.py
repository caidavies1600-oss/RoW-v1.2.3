...
class OwnerActions(commands.Cog):
    ...
    async def sync_discord_members(self, ctx: commands.Context, guild_id: int = None):
        ...
        try:
            ...
            if not (event_manager and hasattr(event_manager, 'data_manager') and hasattr(event_manager.data_manager, 'sheets_manager')):
                return await ctx.send("❌ Google Sheets not configured.")
            
            result = await event_manager.data_manager.sheets_manager.scan_and_sync_all_members(
            ...
            if result["success"]:
                ...
                if event_manager.data_manager.sheets_manager.spreadsheet:
                    embed.add_field(
                        name="🔗 Sheets",
                        value=f"[Open Spreadsheet]({event_manager.data_manager.sheets_manager.spreadsheet.url})",
                        inline=False
                    )
    ...
    async def full_sync_and_setup(self, ctx: commands.Context):
        """Complete setup: sync members and create all templates."""
        try:
            await ctx.send("🚀 **Starting full setup...**\n*This may take 1-2 minutes*")
            
            event_manager = self.bot.get_cog("EventManager")
            if not (event_manager and hasattr(event_manager, 'data_manager')):
                return await ctx.send("❌ Google Sheets not configured.")
            ...
            result = await event_manager.data_manager.sheets_manager.full_sync_and_create_templates(
            ...
            if result["success"]:
                ...
                await ctx.send(f"❌ **Setup failed:** {result.get('error')}")
    ...
    async def sheets_info(self, ctx: commands.Context):
        ...
        try:
            ...
            if not (event_manager and hasattr(event_manager, 'data_manager') and hasattr(event_manager.data_manager, 'sheets_manager') and event_manager.data_manager.sheets_manager):
                embed = discord.Embed(
            ...
            sheets_manager = event_manager.data_manager.sheets_manager
